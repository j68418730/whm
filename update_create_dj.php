<?php
$controllerFile = '/var/www/radiohosting/user/Controllers/RadioController.php';
...
$content = file_get_contents('/var/www/radiohosting/user/Controllers/RadioController.php');

if (strpos($content, 'public function createDj()') !== false) {
    $pattern = '/    public function createDj\(\).+?(?=    public function )/s';
    $content = preg_replace('/    public function createDj\(\).+?(?=    public function )/s', $newCreateDj, $content, 1);
    
    if (strpos($content, 'station_ids') !== false) {
        file_put_contents('/var/www/radiohosting/user/Controllers/RadioController.php', $content);
        echo "SUCCESS: createDj updated with multi-station support\n";
    } else {
        echo "ERROR: Replacement failed\n";
    }
} else {
    echo "ERROR: createDj not found\n";
}