...
if command -v trivy >/dev/null 2>&1; then
    ...
    curl -fsSL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh 2>/dev/null | sh -s -- -b /usr/local/bin 2>/dev/null || {
        sc_log "trivy" "install" "FAIL" "script failed, trying apt/dnf"
        $PKG_INSTALL trivy 2>/dev/null || true
    }
fi
...
sc_status trivy ok "$(trivy --version 2>/dev/null | head -1 || echo installed)"
sc_log "trivy" "install" "OK" "Trivy installed"
