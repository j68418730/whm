<?php
// Read the file
$file = '/var/www/radiohosting/user/Views/radio/index.php';
$content = file_get_contents($file);

// Find the DJ creation form section
$oldForm = '<div class="card"><h3>Add DJ</h3>
...
// New form with station checkboxes
$newForm = '
    ...
    <div style="margin-top:16px;padding:16px;background:rgba(251,146,60,.08);border:1px solid rgba(251,146,60,.2);border-radius:8px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px">
        ...
        <label style="display:flex;align-items:center;gap:8px;padding:8px;background:rgba(0,0,0,.2);border:1px solid rgba(255,255,255,.08);border-radius:6px;cursor:pointer;font-size:13px;color:#e0e0e0;transition:all .15s">
          <input type="checkbox" name="station_ids[]" value="<?php echo $st->id; ?>" ' . $checked . ' ' . $disabled . ' style="margin:0;transform:scale(1.1)">
...
if (strpos($content, '<div class="card"><h3>Add DJ</h3>') !== false) {
    ...
    if (strpos($content, 'Assigned Stations') !== false) {
        ...
        echo "SUCCESS: Form updated with multi-station checkboxes\n";
    } else {
        echo "ERROR: Replacement did not work\n";
    }
} else {
    echo "ERROR: Could not find Add DJ form\n";
}