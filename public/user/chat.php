...
$tenantQ = $pdo->prepare("SELECT * FROM chatbox_tenants WHERE hosting_user_id = ?");
$tenantQ->execute([$hosting->id]);
$tenant = $tenantQ->fetch(PDO::FETCH_OBJ);
...
if ($_POST && isset($_POST['action'])) {
    if ($_POST['action'] === 'create_room') {
        $pdo->prepare("INSERT INTO chatbox_rooms (tenant_id, name, type, password, guest_enabled, registration_enabled, voice_enabled, description, color, icon) VALUES (?,?,?,?,?,?,?,?,?,?)")
            ->execute([$tenant->id, $_POST['name'], $_POST['type'], $_POST['type'] === 'password' ? password_hash($_POST['password'], PASSWORD_DEFAULT) : null, (int)($_POST['guest']??0), (int)($_POST['reg']??0), (int)($_POST['voice']??0), $_POST['description']??'', $_POST['color']??'#008cff', $_POST['icon']??'']);
        header('Location: /user/chat'); exit;
    }
    if ($_POST['action'] === 'update_room') {
        $pdo->prepare("UPDATE chatbox_rooms SET name=?, type=?, password=?, guest_enabled=?, registration_enabled=?, voice_enabled=?, description=?, color=?, icon=? WHERE id=? AND tenant_id=?")
            ->execute([$_POST['name'], $_POST['type'], $_POST['type'] === 'password' ? password_hash($_POST['password'], PASSWORD_DEFAULT) : null, (int)($_POST['guest']??0), (int)($_POST['reg']??0), (int)($_POST['voice']??0), $_POST['description']??'', $_POST['color']??'#008cff', $_POST['icon']??'', (int)$_POST['room_id'], $tenant->id]);
        header('Location: /user/chat'); exit;
    }
    ...
    if ($_POST['action'] === 'save_widget') {
        $pdo->prepare("UPDATE chatbox_tenants SET widget_title=?, widget_color=?, widget_bg=?, widget_text_color=?, widget_border_color=?, widget_glow_color=?, widget_avatar_shape=?, font_family=?, theme=?, custom_css=?, guest_enabled=?, registration_enabled=?, voice_enabled=?, player_html=? WHERE id=?")
            ->execute([$_POST['title'], $_POST['color'], $_POST['bg'], $_POST['text_color'], $_POST['border_color'], $_POST['glow_color'], $_POST['avatar_shape'], $_POST['font'], $_POST['theme'], $_POST['custom_css'], (int)$_POST['guest_all'], (int)$_POST['reg_all'], (int)$_POST['voice_all'], $_POST['player_html'], $tenant->id]);
        header('Location: /user/chat'); exit;
    }
}
...
.room-card .meta{display:flex;gap:12px;font-size:10px;color:var(--text3);margin-bottom:10px}
...
document.getElementById('roomGrid').style.display=tab==='widget'?'none':'';
}
function openCreate(){
document.getElementById('modalTitle').textContent='✏️ Create Room';
document.getElementById('formAction').value='create_room';
document.getElementById('formRoomId').value='';
document.getElementById('fName').value='';document.getElementById('fDesc').value='';
document.getElementById('fType').value='public';document.getElementById('fPass').value='';
document.getElementById('fColor').value='#008cff';document.getElementById('fIcon').value='';
document.getElementById('fGuest').checked=false;document.getElementById('fReg').checked=false;document.getElementById('fVoice').checked=false;
togglePw();document.getElementById('roomModal').classList.add('open');
}
function openEdit(id,name,desc,type,color,icon,guest,reg,voice){
document.getElementById('modalTitle').textContent='⚙️ Edit Room';
document.getElementById('formAction').value='update_room';
document.getElementById('formRoomId').value=id;
document.getElementById('fName').value=name;document.getElementById('fDesc').value=desc;
document.getElementById('fType').value=type;document.getElementById('fPass').value='';
document.getElementById('fColor').value=color||'#008cff';document.getElementById('fIcon').value=icon||'';
document.getElementById('fGuest').checked=guest==1;document.getElementById('fReg').checked=reg==1;document.getElementById('fVoice').checked=voice==1;
togglePw();document.getElementById('roomModal').classList.add('open');
}
...
function copy(btn,mode){
var codes={js:'<script src="//<?=$server?>/chatbox/widget.js.php?tenant_id=<?=$tenant->id?>"><\/script>',iframe:'<iframe src="//<?=$server?>/chatbox/embed.php?tenant_id=<?=$tenant->id?>" width="360" height="500"></iframe>'};
navigator.clipboard.writeText(codes[mode]||'').then(function(){btn.textContent='✅ Copied!';setTimeout(function(){btn.textContent='📋 Copy';},2000);});
}
</script>
