...
<form method="POST"><input type="hidden" name="action" value="create"><button class="btn btn-primary" style="padding:10px 28px;font-size:14px"><i class="bi bi-plus-circle"></i> Enable Chat</button></form>
</div>
<?php else: ?>
...
<div class="embed-row">
<div>
<div style="font-size:10px;color:var(--text3);margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:.3px">JavaScript Widget</div>
<div class="chat-code">&lt;script src="http://<?=$server;?>/chatbox/widget.js.php?tenant_id=<?=$tenant->id;?>"&gt;&lt;/script&gt;</div>
<button class="btn btn-sm" style="background:var(--accent2);color:var(--accent);border:none;border-radius:6px;padding:5px 12px;font-size:10px;font-weight:600;cursor:pointer" onclick="cpt(this,'js')">📋 Copy JS</button>
</div>
<div>
<div style="font-size:10px;color:var(--text3);margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:.3px">iFrame Embed</div>
<div class="chat-code">&lt;iframe src="http://<?=$server;?>/chatbox/embed.php?tenant_id=<?=$tenant->id;?>" width="360" height="500"&gt;&lt;/iframe&gt;</div>
<button class="btn btn-sm" style="background:var(--accent2);color:var(--accent);border:none;border-radius:6px;padding:5px 12px;font-size:10px;font-weight:600;cursor:pointer" onclick="cpt(this,'iframe')">📋 Copy Iframe</button>
</div>
</div>
</div>
...
<?php endif; ?>
<script>
function cpt(btn,mode){
var codes={js:'<script src="http://<?=$server;?>/chatbox/widget.js.php?tenant_id=<?=$tenant->id??0;?>"><\/script>',iframe:'<iframe src="http://<?=$server;?>/chatbox/embed.php?tenant_id=<?=$tenant->id??0;?>" width="360" height="500"></iframe>'};
navigator.clipboard.writeText(codes[mode]||'').then(function(){btn.textContent='✅ Copied!';setTimeout(function(){btn.textContent='📋 Copy '+(mode==='js'?'JS':'Iframe');},2000);});
}
</script>
