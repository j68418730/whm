...
if (!function_exists('user_menu_sections')) {
    function user_menu_sections($features = []): array
    {
        ...
        if (!empty($f->radio) || !empty($f->icecast))
            $sections[] = ['label' => 'Radio', 'href' => '/user/section/radio', 'icon' => '📻', 'match' => ['/user/dj','/user/radio','/user/dj-manager','/dj_panel.php','/user/stats','/user/public-djs','/user/radio/dj-panel','/user/section/radio']];
