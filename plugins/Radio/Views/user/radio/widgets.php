...
<?php $extra = [
  ...
  ['Station Request Widget','station-request','Live DJ, queue & request form for your station'],
...
<script>
var BASE_URL = '<?php echo site_base_url(); ?>';
var STREAM_HOST = '<?php echo primary_domain(); ?>';
var STATIONS = <?=json_encode(array_map(function($s){return['id'=>$s->id,'name'=>$s->name,'port'=>radio_stream_port($s),'type'=>$s->server_type??'icecast','mount'=>$s->mount??'/live'];},$streams))?>;
...
function wc(type){
  ...
  var codes = {
    ...
    'dj-login':'<div style="padding:12px;background:rgba(250,204,21,.06);border:1px solid rgba(250,204,21,.15);border-radius:8px;text-align:center"><div style="font-size:13px;font-weight:600;color:#e0e0e0;margin-bottom:4px">DJ Panel Login</div><div style="font-size:11px;color:#94a3b8;margin-bottom:8px">Give this link to your DJs:</div><input id="dj-login-url" value="'+s+'/dj_panel.php" readonly style="width:100%;padding:8px;border-radius:6px;border:1px solid rgba(255,255,255,.08);background:rgba(0,0,0,.3);color:#4ade80;font-size:11px;text-align:center;font-family:monospace;margin-bottom:6px"><button class="btn-s btn-p" onclick="navigator.clipboard.writeText(document.getElementById(\'dj-login-url\').value)">Copy Link</button></div>',
    'apply':'<div id="ph-apply-'+x+'">DJ application form - create at /user/radio</div>',
    'advertisements':'<div id="ph-ads-'+x+'"><script src="'+s+'/radio/widgets/advertisements.php?stream='+x+'"><\/script><\/div>',
  };
