...
<span class="badge bg-<?php echo !empty($s->live_online) ? 'success' : (($s->status ?? '') === 'suspended' ? 'warning' : 'secondary'); ?>"><?php echo !empty($s->live_online) ? '● Online' : (($s->status ?? '') === 'suspended' ? 'Suspended' : '○ Offline'); ?></span>
</div>
...
<div><span style="color:var(--text_muted)">Listeners:</span> <?php echo (int)($s->live_listeners ?? $s->listener_count ?? 0); ?></div>
