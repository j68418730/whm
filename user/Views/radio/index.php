...
var h='<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:3px;margin-bottom:5px">';
h+='<div style="background:rgba(0,0,0,.3);border-radius:4px;padding:4px;text-align:center"><div style="font-size:11px;font-weight:700;color:'+(d.status==="running"?"#4ade80":"#f87171")+'">'+esc(d.status)+'</div><div style="font-size:6px;color:#64748b">Station</div></div>';
h+='<div style="background:rgba(0,0,0,.3);border-radius:4px;padding:4px;text-align:center"><div style="font-size:11px;font-weight:700;color:'+(d.autodj_running?"#4ade80":"#f87171")+'">'+(d.autodj_running?"Run":"Stop")+'</div><div style="font-size:6px;color:#64748b">AutoDJ</div></div>';
h+='<div style="background:rgba(0,0,0,.3);border-radius:4px;padding:4px;text-align:center"><div style="font-size:11px;font-weight:700;color:'+(d.source_connected?"#4ade80":"#f87171")+'">'+(d.source_connected?"OK":"OFF")+'</div><div style="font-size:6px;color:#64748b">Src</div></div>';
h+='<div style="background:rgba(0,0,0,.3);border-radius:4px;padding:4px;text-align:center"><div style="font-size:11px;font-weight:700;color:'+(d.proxy_reachable?"#4ade80":"#f87171")+'">'+(d.proxy_reachable?d.proxy_response_ms+"ms":"DOWN")+'</div><div style="font-size:6px;color:#64748b">Proxy</div></div>';
h+='<div style="background:rgba(0,0,0,.3);border-radius:4px;padding:4px;text-align:center"><div style="font-size:11px;font-weight:700;color:'+(d.listener_running?"#4ade80":"#f87171")+'">'+(d.listener_running?"ON":"OFF")+'</div><div style="font-size:6px;color:#64748b">Listener</div></div>';
h+='</div>';
...
h+='<div><span style="color:#64748b">Listener PID:</span> <span style="color:#94a3b8">'+(d.listener_pid||'—')+'</span></div>';
h+='</div>';
...
<div id="dj-info-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:999;align-items:center;justify-content:center" onclick="if(event.target===this)this.style.display='none'">
<div style="background:rgba(8,16,28,.95);border:1px solid rgba(56,189,248,.15);border-radius:16px;padding:28px;max-width:440px;width:92%;margin:auto">
<div style="font-size:18px;font-weight:700;margin-bottom:16px;color:#38bdf8">🔌 DJ Connection Info</div>
...
function handleFiles(files){for(var i=0;i<files.length;i++){var f=files[i];var ext=f.name.split('.').pop().toLowerCase();if(['mp3','aac','ogg','flac','wav','m4a','m3u'].indexOf(ext)<0)continue;_queue.push(f);}renderQueue();}
function renderQueue(){_q.innerHTML='';if(!_queue.length){_btn.style.display='none';return;}_btn.style.display='inline-block';_cnt.textContent=_queue.length+' file(s)';for(var i=0;i<_queue.length;i++){var w=document.createElement('div');w.style.cssText='margin:4px 0;padding:4px 8px;background:rgba(0,0,0,.2);border-radius:4px';var n=document.createElement('div');n.style.cssText='font-size:10px;color:#94a3b8;margin-bottom:2px';n.textContent=_queue[i].name+' ('+Math.round(_queue[i].size/1024)+' KB)';w.appendChild(n);var p=document.createElement('div');p.style.cssText='height:3px;border-radius:2px;background:rgba(255,255,255,.06);overflow:hidden';var f=document.createElement('div');f.style.cssText='height:100%;width:0;border-radius:2px;background:linear-gradient(90deg,#008cff,#3bb8ff);transition:width .3s';p.appendChild(f);w.appendChild(p);w._bar=f;w._name=n;_q.appendChild(w);}}
function startUpload(){if(!_queue.length)return;_btn.disabled=true;_btn.textContent='Uploading...';var items=_q.children;var i=0;var total=_queue.length;function uploadNext(){if(i>=total){_btn.textContent='Done';setTimeout(function(){location.reload();},1000);return;}if(items[i])items[i]._name.textContent=_queue[i].name+' ('+Math.round(_queue[i].size/1024)+' KB) - uploading...';var fd=new FormData();fd.append('playlist_id',_playlistId);fd.append('_csrf_token',_csrf);fd.append('files[]',_queue[i]);var x=new XMLHttpRequest();x.open('POST','/user/radio/media/upload',true);x.setRequestHeader('X-CSRF-Token',_csrf);x.upload.onprogress=function(ev){if(ev.lengthComputable&&items[i]){var pct=Math.round(ev.loaded/ev.total*100);items[i]._bar.style.width=pct+'%';items[i]._name.textContent=_queue[i].name+' ('+Math.round(_queue[i].size/1024)+' KB) - '+pct+'%';}};x.onload=function(){if(x.status===200&&items[i]){items[i]._bar.style.width='100%';items[i]._name.textContent=_queue[i].name+' - done';i++;uploadNext();}else if(items[i]){items[i]._name.textContent=_queue[i].name+' - failed';_btn.disabled=false;}};x.send(fd);}uploadNext();}
</script>