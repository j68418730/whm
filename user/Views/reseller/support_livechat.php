...
<div class="tab" onclick="switchTab('embed',this)">🔗 Embed / Enable</div>
</div>
...
<div style="font-size:11px;color:var(--text-muted)">#<?php echo $s->id; ?> · <?php echo htmlspecialchars($s->department); ?> · Client: <?php echo htmlspecialchars($s->client_name); ?></div>
</div>
<?php endforeach; ?>
...
<p style="color:var(--text-muted);font-size:13px;margin:0 0 8px">Copy and paste this code into any HTML page to add the live chat widget for <strong><?php echo htmlspecialchars($embedDomain); ?></strong>:</p>
...
(function() {
  ...
  s.src = "https://' . $embedDomain . '/chatbox/widget.js.php";
  s.async = true;
  s.onload = function() {
...
<div class="card">
<h4 style="margin:0 0 12px;font-size:14px">🛡️ Enable Live Chat on Your Domain</h4>
<p style="color:var(--text-muted);font-size:13px;margin:0 0 12px">Toggle live chat for <strong><?php echo htmlspecialchars($embedDomain); ?></strong>. When enabled, visitors will see the chat widget.</p>
<form method="POST" action="/reseller/support-chat/toggle-enabled" style="display:flex;align-items:center;gap:8px">
<input type="hidden" name="domain" value="<?php echo htmlspecialchars($embedDomain); ?>">
<input type="hidden" name="enabled" value="0">
<input type="checkbox" name="enabled" value="1" id="liveChatEnabled" <?php echo true ? 'checked' : ''; ?> style="width:20px;height:20px">
<label for="liveChatEnabled" style="cursor:pointer;font-size:13px;color:#e0e0e0">Enable Live Chat Widget</label>
<button type="submit" class="btn btn-sm primary" style="margin-left:auto">Save</button>
</form>
</div>
</div>
...
(function() {
  ...
  x.open("POST", "https://' . $embedDomain . '/admin/livechat/track", true);
...
<h4 style="margin:0 0 12px;font-size:14px">📋 Quick Links</h4>
<div class="canned-item"><a href="/reseller/support-system" style="color:var(--accent);text-decoration:none">⬅ Back to Support System</a></div>
<div class="canned-item"><a href="/reseller/clients" style="color:var(--accent);text-decoration:none">👥 View Clients</a></div>
<div class="canned-item"><a href="/reseller/provisioning" style="color:var(--accent);text-decoration:none">⚙️ Provisioning</a></div>
</div>
</div>
...
function refreshChatSidebar() {
    var x = new XMLHttpRequest();
    x.open('GET', '/reseller/support-chat', true);
    x.onload = function() {
...
function startVisitorChat(visitorId, name) {
    var x = new XMLHttpRequest();
    x.open('POST', '/reseller/support-chat/initiate', true);
    x.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    x.onload = function() {
        try {
...
function fetchMessages() {
    ...
    var x = new XMLHttpRequest();
    x.open('GET', '/reseller/support-chat/messages/' + currentSession + '?since=' + lastMsgId, true);
    x.onload = function() {
        if (x.status !== 200) return;
        try {
...
function sendMsg() {
    ...
    var x = new XMLHttpRequest();
    x.open('POST', '/reseller/support-chat/send', true);
    x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
...
function closeChat() {
    ...
    var x = new XMLHttpRequest();
    x.open('GET', '/reseller/support-chat/close/' + currentSession, true);
...
function transferChat() {
    ...
    var x = new XMLHttpRequest();
    x.open('POST', '/reseller/support-chat/transfer/' + currentSession, true);
    x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
