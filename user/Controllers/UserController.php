...
class UserController extends Controller
{
    ...
    public function djPanel() { $u = $this->loadUser(); $streams = []; if ($this->hostingUser) { try { $streams = $this->db->table('radio_streams')->where('user_id', $this->hostingUser->id)->get() ?: []; if (empty($streams) && $this->package && ($this->package->icecast_enabled ?? 0)) { $sid = $this->db->table('radio_streams')->insertGetId(['user_id' => $this->hostingUser->id, 'server_type' => 'icecast', 'port' => 8000, 'password' => substr(md5(time()), 0, 8), 'config_path' => '/etc/icecast/radiohosting', 'status' => 'stopped']); $streams = $this->db->table('radio_streams')->where('id', $sid)->get() ?: []; } } catch(\Exception $e) {} } return $this->view('user.dj_dashboard', ['user' => $u, 'hosting' => $this->hostingUser, 'streams' => $streams, 'title' => 'DJ Panel']); }
    public function publicDjs() { $u = $this->loadUser(); return $this->view('user.public_djs', ['user' => $u, 'hosting' => $this->hostingUser, 'title' => 'Our DJs']); }
