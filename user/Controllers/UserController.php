...
class UserController extends Controller
{
    ...
    public function ftp() { $u = $this->loadUser(); $accounts = []; if ($this->hostingUser) { try { $accounts = $this->db->table('ftp_accounts')->where('hosting_user_id', $this->hostingUser->id)->get() ?: []; } catch (\Exception $e) {} } return $this->view('user.ftp', ['user' => $u, 'hosting' => $this->hostingUser, 'ftpAccounts' => $accounts, 'title' => 'FTP Accounts']); }
    public function ftpCreate() { $u = $this->loadUser(); if (!$this->hostingUser) { header('Location: /user/ftp'); exit; } $username = strtolower(preg_replace('/[^a-z0-9]/', '', $_POST['username'] ?? '')); $password = $_POST['password'] ?? ''; $dir = trim($_POST['directory'] ?? '/'); if (!$username || !$password) { $_SESSION['error'] = 'Username and password required.'; header('Location: /user/ftp'); exit; } $fullUser = $this->hostingUser->username . '_' . $username; try { $this->db->table('ftp_accounts')->insertGetId(['hosting_user_id' => $this->hostingUser->id, 'username' => $fullUser, 'password_hash' => password_hash($password, PASSWORD_DEFAULT), 'directory' => $dir]); $_SESSION['success'] = "FTP user '{$fullUser}' created."; @exec("sudo useradd -m -d /home/{$this->hostingUser->username}/{$dir} -s /bin/bash {$fullUser} 2>/dev/null && echo '{$password}' | sudo passwd --stdin {$fullUser} 2>/dev/null"); } catch (\Exception $e) { $_SESSION['error'] = 'Failed to create FTP user.'; } header('Location: /user/ftp'); exit; }
    public function ftpDelete($id) { $u = $this->loadUser(); if ($this->hostingUser) { try { $acct = $this->db->table('ftp_accounts')->where('id', $id)->where('hosting_user_id', $this->hostingUser->id)->first(); if ($acct) { @exec("sudo userdel -rf {$acct->username} 2>/dev/null"); $this->db->table('ftp_accounts')->where('id', $id)->delete(); $_SESSION['success'] = 'FTP user deleted.'; } } catch (\Exception $e) {} } header('Location: /user/ftp'); exit; }
