...
class GameServersController extends Controller
{
    ...
    // ─── Game Templates (SteamCMD install recipes) ───
    ...
    public function templates()
    {
        ...
        $cat = trim($this->request->get('category', ''));
        $builder = $this->db->table('game_templates')->orderBy('name', 'ASC');
        $templates = $builder->get() ?: [];
        if ($q !== '') {
            $templates = array_filter($templates, function ($t) use ($q) {
                return stripos($t->name ?? '', $q) !== false || stripos($t->appid ?? '', $q) !== false || stripos($t->category ?? '', $q) !== false;
            });
        }
        if ($cat !== '') {
            $templates = array_filter($templates, function ($t) use ($cat) { return ($t->category ?? '') === $cat; });
        }
        $categories = $this->db->pdo()->query("SELECT category, COUNT(*) AS cnt FROM game_templates GROUP BY category ORDER BY category")->fetchAll(\PDO::FETCH_OBJ);
        $engines = $this->db->pdo()->query("SELECT DISTINCT engine FROM game_templates WHERE engine IS NOT NULL AND engine != '' ORDER BY engine")->fetchAll(\PDO::FETCH_COLUMN);
        return $this->view('admin.gameservers.templates', [
            'user' => $user,
    ...
    public function templatesStore()
    {
        ...
        $data = [
            ...
            'install_script' => $this->request->post('install_script', ''),
            'start_command' => $this->request->post('start_command', ''),
            'stop_command' => $this->request->post('stop_command', ''),
            'restart_command' => $this->request->post('restart_command', ''),
            ...
            'status' => $this->request->post('status', 'active') === 'active' ? 'active' : 'inactive',
        ];
        if (!$data['name'] || !$data['appid']) {
            $_SESSION['error_message'] = 'Name and AppID are required.';
            $this->response->redirect('/admin/games/templates'); exit;
        }
        if ($id) {
    ...
    public function templatesToggle($id)
    {
        $this->guard();
        $t = $this->db->table('game_templates')->where('id', (int)$id)->first();
        if (!$t) { $_SESSION['error_message'] = 'Template not found.'; $this->response->redirect('/admin/games/templates'); exit; }
        $new = $t->status === 'active' ? 'inactive' : 'active';
        $this->db->table('game_templates')->where('id', (int)$id)->update(['status' => $new]);
        $_SESSION['success_message'] = $new === 'active' ? 'Template activated.' : 'Template deactivated.';
        $this->response->redirect('/admin/games/templates');
    }
    ...
    public function templatesDelete($id)
    {
        ...
        $_SESSION['success_message'] = "Template '{$t->name}' deleted.";
        $this->response->redirect('/admin/games/templates');
    }

    public function templatesCatalog()
    {
        $this->guard();
        // JSON list (used by install wizards / provisioning)
        $templates = $this->db->table('game_templates')->where('status', 'active')->orderBy('name', 'ASC')->get() ?: [];
        $this->response->json($templates)->send();
        exit;
    }
    ...
    public function templateView($id)
    {
        $this->guard();
        $t = $this->db->table('game_templates')->where('id', (int)$id)->first();
        if (!$t) { $this->response->json(['error' => 'Template not found'])->json(); exit; }
        $this->response->json($t)->send();
        exit;
    }
