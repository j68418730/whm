...
class CloudPanelMigrationAdapter extends BaseMigrationAdapter {
    public function getPanelName(): string { return 'CloudPanel'; }
    public function getPanelIcon(): string { return '☁️'; }
    ...
    public function testConnection(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        return $this->httpGet("https://{$host}:{$port}/api/sites", ["Authorization: Bearer {$password}"]);
    }
    public function preflight(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        $resp = $this->httpGet("https://{$host}:{$port}/api/sites", ["Authorization: Bearer {$password}"]);
        $data = json_decode($resp['body'], true);
        $accounts = [];
        foreach ($data['sites'] ?? $data ?? [] as $s) { $accounts[] = ['username' => $s['domain'] ?? $s['name'] ?? 'unknown', 'domain' => $s['domain'] ?? '', 'plan' => $s['plan'] ?? '']; }
        return ['connected' => true, 'accounts' => $accounts, 'server_info' => ['cloudpanel' => true, 'total' => count($accounts)]];
    }
