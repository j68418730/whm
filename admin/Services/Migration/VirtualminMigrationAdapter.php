...
class VirtualminMigrationAdapter extends BaseMigrationAdapter {
    public function getPanelName(): string { return 'Virtualmin'; }
    ...
    public function testConnection(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        $resp = $this->httpGet("https://{$host}:{$port}/virtualmin/rest/list-domains?json=1", ["Authorization: Basic " . base64_encode("{$username}:{$password}")]);
    ...
    public function preflight(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        ...
        $accounts = [];
        foreach ($data['data'] ?? [] as $d) { $accounts[] = ['username' => $d['name'] ?? 'unknown', 'domain' => $d['domain'] ?? '', 'plan' => $d['plan'] ?? '']; }
        return ['connected' => true, 'accounts' => $accounts, 'server_info' => ['virtualmin' => true, 'total' => count($accounts)]];
    }
