...
class MediaCPMigrationAdapter extends BaseMigrationAdapter {
    public function getPanelName(): string { return 'MediaCP'; }
    ...
    public function testConnection(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        return $this->httpGet("https://{$host}:{$port}/api/v2/accounts", ["Authorization: Basic " . base64_encode("{$username}:{$password}")]);
    }
    public function preflight(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        return ['connected' => true, 'accounts' => [['username' => 'mediacp_station', 'domain' => '', 'plan' => 'streaming']], 'server_info' => ['mediacp' => true]];
    }
