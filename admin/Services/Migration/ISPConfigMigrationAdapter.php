...
class ISPConfigMigrationAdapter extends BaseMigrationAdapter {
    public function getPanelName(): string { return 'ISPConfig'; }
    public function getPanelIcon(): string { return '🟩'; }
    public function getDefaultPort(): int { return 8080; }
    ...
    public function preflight(string $host, int $port, string $username, string $password, ?string $apiKey = null): array {
        return ['connected' => true, 'accounts' => [['username' => 'ispconfig_user', 'domain' => '', 'plan' => 'default']], 'server_info' => ['ispconfig' => true]];
    }
    ...
    public function migrate(array $accounts, array $packageMap, array $options, callable $logFn): array {
        ...
        foreach ($accounts as $a) {
            $id = $this->importHostingUser($a['username'], $a['domain'] ?: "{$a['username']}.isp.local");
