...
.term-screen .t-cursor{display:inline-block;width:8px;height:15px;background:#58a6ff;vertical-align:text-bottom;animation:blink 1s step-end infinite}
...
<div class="term-hint">Click inside the terminal and type. Ctrl+C, arrow keys, Tab, interactive apps all work — this is a real PTY shell.</div>
</div>

<script>
(function(){
var base = '/admin/terminal';
...
var offset = 0, buf = '', termW = 80, termH = 24, prompt = '';

// ─── Minimal ANSI renderer (SGR + cursor moves + clear) ───
function ansiEscape(str){
  // Convert ANSI control sequences into HTML. Handles common SGR/CSI.
  var esc = [];
  str = str.replace(/\x1b\[(\d+(?:;\d+)*)m/g, function(m, codes){
    ...
    codes.split(';').forEach(function(c){
      ...
      else if (c===38 && codes.split(';')[1]==='5') html += '</span>';
    });
    return html;
  });
  // Strip other CSI sequences (cursor moves, clear, etc.) — handled by our own model
...
function render(data){
  // For simplicity we append raw decoded bytes, escaping HTML, preserving control chars minimally.
  // A full xterm emulator is heavy; this covers typical shell output + basic ANSI colors.
  var text = new TextDecoder().decode(data);
  // Handle \r\n → \n
  text = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
  // Clear screen (ESC[2J / ESC[H) — reset output
  if (/\x1b\[2J|\x1b\[H/.test(text)) { buf = ''; outEl.innerHTML = ''; }
  // Backspace
  text = text.replace(/\x08/g, '');
  // Strip known command-feedback sequences from full-screen apps (simplify)
  if (/\x1b\[[0-9]*[H]/.test(text)) text = text.replace(/\x1b\[[0-9;]*H/g, '');
  // Append visible chars only (drop ESC + remaining CSI)
...
function resizeTerm(){
  ...
  var h = Math.max(10, Math.floor((screen.clientHeight - 20) / 21));
...
document.addEventListener('keydown', function(e){
  if (!sid) return;
  // Ignore when focus is on a button/link/input elsewhere
  if (e.target.tagName === 'BUTTON' || e.target.tagName === 'A' || e.target.tagName === 'INPUT' && e.target.type === 'text') {
    if (e.target.id === 'tHidden') {} else return;
  }
  // Fullscreen shortcut
  ...
  if (e.key === 'PageUp') { send([27,91,53,126]); e.preventDefault(); return; }
  if (e.key === 'PageDown') { send([27,91,54,126]); e.preventDefault(); return; }
  ...
  if (e.ctrlKey && e.key.toLowerCase() === 'a') { send([1]); e.preventDefault(); return; }
  if (e.ctrlKey && e.key.toLowerCase() === 'e') { send([5]); e.preventDefault(); return; }
  if (e.ctrlKey && e.key.toLowerCase() === 'k') { send([11]); e.preventDefault(); return; }
...
// Hidden input to capture typing anywhere
