<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Licensing - Radio Hosting Panel</title>
    <link rel="stylesheet" href="/css/admin.css">
</head>
<body class="whm-body">
    <main class="whm-shell">
        <aside class="whm-sidebar">
            <div class="brand"><span class="brand-mark">S</span><div><strong>Spectre WHM</strong><small>Hosting + Radio</small></div></div>
            <a href="/admin/dashboard">Dashboard</a>
            <a href="/admin/account">Account Functions</a>
            <a href="/admin/reseller">Reseller Center</a>
            <a href="/admin/packages">Packages</a>
            <a href="/admin/streams">Radio Streams</a>
            <a href="/admin/radio_dashboard">Radio Dashboard</a>
            <a href="/admin/server">Server Overview</a>
        </aside>
        <section class="whm-content">
            <div class="module-header">
                <span class="eyebrow">WHM Module</span>
                <h1>Licensing</h1>
                <p>Manage licensing configuration and settings from this panel.</p>
            </div>
            <div class="card-grid">
                <article class="module-card"><h3>Overview</h3><p>This module provides full licensing management capabilities.</p></article>
                <article class="module-card"><h3>Configuration</h3><p>Configure and customize licensing settings for your hosting environment.</p></article>
            </div>
        </section>
    </main>
</body>
</html>
