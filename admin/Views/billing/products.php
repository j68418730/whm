...
.bp-card .drag-handle{position:absolute;top:8px;right:8px;color:#64748b;cursor:grab;display:none}
.bp-card.sorting .drag-handle{display:inline}
...
<button class="btn secondary" onclick="toggleSort()" id="sortToggleBtn">↕ Sort</button>
</div>
...
<form method="POST" action="/admin/billing/products/store" id="bpForm">
...
function openAdd() {
    ...
    document.getElementById('bpForm').action = '/admin/billing/products/store';
...
function toggleSort() {
    ...
    document.querySelectorAll('.bp-card').forEach(function(c){
        c.classList.toggle('sorting', sortMode);
        c.setAttribute('draggable', sortMode ? 'true' : 'false');
    });
...
document.addEventListener('dragstart', function(e){
    if (!sortMode) return;
    dragEl = e.target.closest('.bp-card');
...
document.addEventListener('dragover', function(e){
    if (!sortMode) return;
    var target = e.target.closest('.bp-card');
...
function saveSortOrder() {
    var ids = [];
    document.querySelectorAll('.bp-card[data-id]').forEach(function(c){ ids.push(c.getAttribute('data-id')); });
    var x = new XMLHttpRequest();
    x.open('POST','/admin/billing/products/sort',true);
    x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
    x.onload=function(){ if(x.status===200 && typeof showToast==='function') showToast('Sort order saved'); };
    x.send('ids=' + ids.join(','));
}
</script>
