...
<div class="form-group"><label>Hostname</label><input name="hostname" value="<?php echo htmlspecialchars($hostname); ?>">
...
<button type="submit" class="btn btn-sm primary"><i class="bi bi-plus-circle"></i> Add IP</button>
</form>
<table>
...
<td><a href="/admin/ip/delete/<?php echo $ip->id; ?>" class="btn btn-sm" style="background:rgba(248,113,113,.12);color:#f87171" onclick="return confirm('Delete this IP?')"><i class="bi bi-trash"></i></a></td>
</tr>
<?php endforeach; else: ?>
...
<div class="form-group"><label>Timezone</label><select name="timezone"><?php foreach (timezone_identifiers_list() as $tz): ?><option value="<?php echo $tz; ?>" <?php echo $tz===$timezone?'selected':''; ?>><?php echo $tz; ?></option><?php endforeach; ?></select>
<div class="card" style="margin-top:20px">
<h3 style="color:var(--accent);margin-bottom:16px">🌐 Server IPs</h3>
<p style="color:#64748b;font-size:13px;margin-bottom:12px">Manage server IP addresses and see which are in use.</p>
<?php
